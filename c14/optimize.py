...
from .tools import RK45,ImplicitParametersOutOfRange,NonFiniteValuesinIntegrate
...
class optimize():
    def __init__(self, model, data, use_error=False, step_size=0.1, rtol=0.1, min_step=0.001):
        self.model = model
        self.data = data
    ...
    def calc_sim_data_dict(self, parameters_fit, model=None,mode='freq'):
        if model is None:
            model = self.model
        return self.calc_sim_data([parameters_fit[i] for i in model.parameter_names],model,mode)

    def calc_sim_data(self, parameters_fit, model,mode='freq'):
        model.set_Dbirth(self.data.Dbirth)
        if not model.set_parameters_fit_array(parameters_fit,mode=mode):
            return -np.inf
    ...
    def odeint(self, model):
        ...
        try:
            ...
            logger.debug("Value Error in ode int: %s", e)
            return None
        except ImplicitParametersOutOfRange as e:
            logger.debug("ImplicitParametersOutOfRange Error in ode int: %s ;  return none", e)
            return None
        except NonFiniteValuesinIntegrate as e:
            logger.debug("Error in ode int: %s ;  return none", e)
            return None
            
        if sol['status'] < 0:
            logger.debug("ode int failed with:  %s", sol['message'])
            return None
        if np.isnan(sol['y']).any():
            logger.debug("NaN in ode int solution")
            return None
    ...
    def optimize_minuit_multistart(self, processes=4, return_with_error=True, mode='uniform',perror=None, init_limit=None, n=100, seed=42,\
        nwalkers=100, steps=100, method=None):
        #ingnore sigma
        parameter_names = self.model.parameter_names.copy()
        parameter_names.remove('sigma')
        if perror is None:
        ...
        with Pool(processes=processes) as pool:
            res_pool = [pool.apply_async( self.optimize_minuit,(p0, error,None, method)  ) for p0 in p0s]
    ...
    def optimize_minuit(self, pin=None, perror=None, plimit=None,method=None):
        #ignore sigma
        parameter_names = self.model.parameter_names.copy()
        parameter_names.remove('sigma')
        ...
        try:
            res = sp.optimize.minimize(lambda x: self.Nloglike(x, copy.deepcopy(self.model)), p0,method=method)
            ...
            logger.debug("Optimization failed: %s", e)
    ...
    def lnprob(self, parameters, model):
        ...
        try:
            ...
            logger.debug("Value Error in ode int: %s ; return -inf", e)
            if not self.gradient_to_default:
                return -np.inf,np.ones(data_elements)*-np.inf
            else:
                interpolate_to_default_para = np.array([i(f) for i, f in zip(self.interpolate_ll, parameters)]).sum()
            ...
            logger.debug("ImplicitParametersOutOfRange Error in ode int: %s ;  return inf", e)
            if not self.gradient_to_default:
                return -np.inf,np.ones(data_elements)*-np.inf
            else:
                interpolate_to_default_para = np.array([i(f) for i, f in zip(self.interpolate_ll, parameters)]).sum()
        ...
        if sol['status'] < 0:
            logger.debug("ode int failed with:  %s ;  return -inf",
                        sol['message'])
            if not self.gradient_to_default:
                return -np.inf,np.ones(data_elements)*-np.inf
            else:
        ...
        if np.isnan(sol['y']).any() or np.isinf(sol['y']).any():
            logger.debug("NaN in ode int solution; return -inf")
            if not self.gradient_to_default:
                return -np.inf,np.ones(data_elements)*-np.inf
            else:
                interpolate_to_default_para = np.array([i(f) for i, f in zip(self.interpolate_ll, parameters)]).sum()
        ...
        if model.limit['sigma'][0] > sigma or sigma > model.limit['sigma'][1]:
            logger.debug("sigma is outside its limits; return -inf")
            if not self.gradient_to_default:
                return -np.inf,np.ones(data_elements)*-np.inf
            else:
                interpolate_to_default_para = np.array([i(f) for i, f in zip(self.interpolate_ll, parameters)]).sum()
                return interpolate_to_default_para,np.ones(data_elements)*interpolate_to_default_para
        if self.use_error:
            sigmasqu = sigma*sigma + self.data.df.sigma14C.values*self.data.df.sigma14C.values
            l_sigmasqu = np.log(sigmasqu)
            s_sigmasqu = 1/(2*sigmasqu)
            loglike = -0.9189385332046727*n - 0.5*np.sum(l_sigmasqu) - np.sum(s_sigmasqu*ressqu)
            loglike_i = -0.9189385332046727 - 0.5*l_sigmasqu - s_sigmasqu*ressqu
        else:
        ...
        P = model.calc_prior()
        return np.log(P)+loglike,loglike_i
        

    def optimize_emcee(self, pin=None, mode="uniform",seed=42, error=None, steps=500, nwalkers=None, processes=1,init_fit_limit=None,fake_gradient=False,m_max=100,**opt_args):
        ...
        if mode == "uniform":
            ...
            for _w in range(nwalkers):
                ...
                p0s.append(tmp)
        elif mode=='uniform_test':
            p0s = []
            m=0
            while (len(p0s)<nwalkers):
                m=m+1
                if (m >= m_max):
                    print(p0s)
                    raise Exception(f"found only  {len(p0s)} valid starting values in {m} runs")
                if init_fit_limit is None:
                    phy_limit = self.model.limit
                    lower_phy_limit = {k:phy_limit[k][0] for k in phy_limit.keys()}
                    upper_phy_limit = {k:phy_limit[k][1] for k in phy_limit.keys()}
                    ...
                    fit_limit_list = [init_fit_limit[pname] for pname in parameter_names]

                p0s_untested = np.random.uniform([l[0] for l in fit_limit_list], [l[1] for l in fit_limit_list], (nwalkers, self.model.nparas)  )
                for p0 in p0s_untested:
                    p0_dict ={name:val for name,val in zip(self.model.parameter_names,p0)}
                    if self.model.set_parameters_fit(p0_dict, mode='bayes'):
                        try:
                            [self.model.calc_implicit_parameters(i) for i in np.arange(0,100,1)]
                        except ImplicitParametersOutOfRange as e:
                            logger.debug(e)
                            continue
                        p0s.append(p0)
            ...
            raise NotImplementedError
        logger.info("Finished Init starting EMCEE")
        with Pool(processes=processes) as pool:
        ...
        return None, chain, lnprobability , lnprobability_i
